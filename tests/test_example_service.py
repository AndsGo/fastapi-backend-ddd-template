...
def test_create_example_item_use_case_creates_item() -> None:
    repository = FakeExampleRepository()

    item = CreateExampleItemUseCase(repository).execute(
        CreateExampleItemCommand(code="new-code", name="New Item", description="Example"),
    )
...
def test_create_example_item_use_case_rejects_duplicate_code() -> None:
    repository = FakeExampleRepository()

    with pytest.raises(ConflictError):
        CreateExampleItemUseCase(repository).execute(
            CreateExampleItemCommand(code="sample", name="Duplicate")
        )


def test_get_example_item_use_case_raises_not_found() -> None:
    repository = FakeExampleRepository()

    with pytest.raises(NotFoundError):
        GetExampleItemUseCase(repository).execute(999)


def test_update_example_item_use_case_updates_item() -> None:
    repository = FakeExampleRepository()

    item = UpdateExampleItemUseCase(repository).execute(1, UpdateExampleItemCommand(name="Renamed"))
