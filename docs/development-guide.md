...
```bash
python -m app.interface.jobs.runner scheduler --interval-seconds 10 --scheduler-id scheduler-1
```
...
```bash
python -m app.interface.jobs.runner all --interval-seconds 10 --max-workers 4
```
...
```powershell
$env:LOG_OUTPUT = "file"; $env:LOG_FILE_PATH = "logs/app.log"; python -m app.interface.jobs.runner worker --once
```

Use both stdout and file:

```powershell
$env:LOG_OUTPUT = "both"; python -m app.interface.jobs.runner worker --once
```
